...
export function TokenCandleChart({ 
  ...
  // Get theme colors
  const { getThemeColor } = useChartTheme()
  ...
  const drawChart = useCallback(() => {
    ...
    data.forEach((candle, i) => {
      ...
      ctx.strokeStyle = candle.close >= candle.open ? getThemeColor(0) : getThemeColor(1)
      ctx.stroke()

      // Draw body
      ctx.fillStyle = candle.close >= candle.open ? getThemeColor(0) : getThemeColor(1)
  ...
  useEffect(() => {
    ...
    // Observe the document root for class changes (theme updates)
