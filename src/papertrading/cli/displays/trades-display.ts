...
export function displayRecentTrades(trades: SimulatedTrade[], style: DashboardStyle): void {
    ...
    const rows = trades.map((trade: SimulatedTrade) => {
        ...
        return [
            ...
            (trade.dex_data?.marketCap || '0').toString().padStart(columnWidths[7]),
            (trade.dex_data?.liquidity_buy_usd || '0').toString().padStart(columnWidths[8]),
            (trade.time_sell ? (trade.dex_data?.liquidity_sell_usd || '0').toString() : '-').padStart(columnWidths[9]),
            (trade.pnl ? trade.pnl.toString(8) : '-').padStart(columnWidths[10])
        ];
    });
