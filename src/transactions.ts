 ...
 * Provides functionality for creating swap transactions, fetching transaction details,
 * and performing rug checks on tokens. Integrates with Jupiter API for swaps and
 * RugCheck API for token validation.
 */
...
/**
 * Fetches detailed information about a transaction using its signature.
 * @param signature - The transaction signature to fetch details for
 * @returns Promise<MintsDataReponse | null> - Transaction details including token mints, or null if fetch fails
 */
...
export async function getRugCheckConfirmed(tokenMint: string, retryCount = 0): Promise<boolean> {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error('🚨 RugCheck API Error:', {
      ...
      // Retry on 502 Bad Gateway or other potentially temporary errors
      if (retryCount < maxRetries && 
        ...
        console.log(`⏳ RugCheck API error, retrying in ${delayMs/1000} seconds... (Attempt ${retryCount + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, delayMs));
        return getRugCheckConfirmed(tokenMint, retryCount + 1);
      }
    }
    console.error('❌ Failed to perform rug check after all retries');
    return false;
  }
}
